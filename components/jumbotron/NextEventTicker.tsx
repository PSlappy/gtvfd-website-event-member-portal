  ...
  return (
    <div className="grid grid-cols-3 bg-gt-navy">
      ...
      <div className="gt-depth-panel relative flex h-full flex-col items-center justify-between border-r-[1px] border-gt-gray-light px-2 py-3 text-center sm:border-r-2 sm:py-4">
        {/* Row 1: date and kickoff time together, larger font. Just the
            time itself, never the literal word "Kickoff" — per the
            owner, that label should never actually appear on screen. */}
        <span className="gt-led-text-white gt-display-in text-xs font-bold text-white sm:text-sm">
          {formatGameDate(game.date)} &bull; {game.kickoff}
        </span>
        ...
        {/* Row 3: sign-up — only if a tailgate event exists for this game.
            mt-3 on top of the column's own justify-between spacing, per
            the owner: specifically more room between the tailgate time
            above and this button, not just the even 3-way split. */}
        {game.tailgate && (
          <div className="gt-display-in mt-3 [animation-delay:160ms]">
